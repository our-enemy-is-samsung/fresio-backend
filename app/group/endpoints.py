...
class GroupEndpoint:
    ...
    @inject
    async def get_team_list(self, user: User = Depends(get_current_user_entity)): ...

    @router.get(
        "/{group_id}/list", description="그룹에 있는 서브 그룹 목록을 반환합니다."
    )
    @inject
    async def get_sub_group_list(self): ...

    @router.get(
        "/{group_id}/{subgroup_id}/list",
        description="하위 그룹에 있는 학생을 반환합니다.",
    )
    @inject
